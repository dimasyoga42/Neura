...
export const Spotifysearch = async (sock, chatId, msg, text) => {
  try {
    const lagu = text.replace(".spotify", "").trim();
    if (!lagu)
    ...
    const response = await axios.get(
      `https://api.deline.web.id/downloader/spotifyplay?q=${decodeURIComponent(lagu)}`,
    );
...
judul lagu: ${data.result.metadata.title}\nartis: ${data.result.metadata.artist}\ndurasi: ${data.result.metadata.duration}
    ...
    sock.sendMessage(
      chatId,
      { image: { url: `${data.result.metadata.cover}` }, caption: messagetxt },
      { quoted: msg },
    );
    sock.sendMessage(
      chatId,
      { audio: { url: `${data.result.dlink}` }, mimetype: "audio/mp4" },
      { quoted: msg },
    );
  } catch (error) {}
};
